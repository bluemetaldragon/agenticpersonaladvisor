"""TR-PRE: pre-read generation. (Updated for the committee->lens fold.)"""
from __future__ import annotations

from app.domain import DirectorProfile, RetentionMode
from app.engine.engine import BoardPrepEngine
from tests.conftest import make_providers, SpyResearch


def _run(pdf, lens_id):
...
def test_preread_makes_no_external_calls(synthetic_pdf):  # TR-PRE-2
    ...
    engine.ingest_and_preread(
        synthetic_pdf, "Q3", DirectorProfile(lens_id="lens_risk"), RetentionMode.PERSIST)
    assert spy.calls == []